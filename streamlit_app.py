...
# Estilos CSS personalizados para mejorar el acabado visual
st.markdown(
    """
    <style>
    .stMetric {
        ...
        if seccion_admin == "📊 Flujo de Caja":
            try:
                if not df_existente.empty:
                    ...
                    # Tarjetas Principales del Negocio
                    ...
                    # Distribución Visual con Plotly y Tarjetas de Cuentas
                    ...
                    with col_cuentas:
                        with st.container(border=True):
                            ...
                            st.metric("💵 Efectivo Físico", f"${efectivo_total:,.2f}")
                            st.metric("📱 Pago Móvil", f"${pago_movil_total:,.2f}")
                            st.metric("🪙 Binance (Crypto)", f"${binance_total:,.2f}")
            ...
            with st.container(border=True):
                ...
                if not usar_dos_cuentas:
                    cuenta_afectada = st.selectbox(
                        "Cuenta asociada:", ["Efectivo", "Pago Móvil", "Binance"]
                    )
                ...
                if not es_nuevo_cliente and opciones_clientes:
                    ...
                    nuevo_nombre = col_nc2.text_input("Nombre Completo", key="nc_nom")
                ...
                with st.form("form_nuevo_registro", border=False):
                    ...
                    if usar_dos_cuentas:
                        ...
                        monto_c1 = col_m1.number_input(
                            f"Monto ({c_1}) ($)", min_value=0.0, value=0.0, key="mc1"
                        )
                        ...
                        c_2 = col_m2.selectbox("Segunda Cuenta", otras_cuentas, key="c2")
                        monto_c2 = col_m2.number_input(
                            f"Monto ({c_2}) ($)", min_value=0.0, value=0.0, key="mc2"
                        )
                    ...
                    if tipo_movimiento == "Registrar Préstamo / Deuda Inicial":
                        ...
                        valor_cuota_calc = (
                            deuda_total_calc / num_cuotas if num_cuotas > 0 else 0.0
                        )
        ...
        # ------------------------------------------
            ...
            with st.container(border=True):
                ...
                with st.form("form_inyectar"):
                    ...
                    miny = st.number_input("Monto a Inyectar ($)", min_value=0.0)
                    din = st.text_input("Nota / Origen de los fondos")

                    if st.form_submit_button("Ingresar Fondos", use_container_width=True):
                        if miny > 0:
                            try:
                                scope = [
                                    "https://www.googleapis.com/auth/spreadsheets",
                                ...
                                sheet.append_row(fila)
                                st.toast("✅ Capital ingresado con éxito", icon="🎉")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error: {e}")
        ...
        # ------------------------------------------
            ...
            with st.container(border=True):
                ...
                with st.form("form_trans"):
                    ...
                    cor = st.selectbox(
                        "Origen (Sale)", ["Efectivo", "Pago Móvil", "Binance"]
                    )
                    cde = st.selectbox(
                        "Destino (Entra)", ["Pago Móvil", "Efectivo", "Binance"]
                    )
        ...
        # ------------------------------------------
            ...
            with st.container(border=True):
                ...
                if opciones_clientes:
                    ...
                    if st.button(
                        "✂️ Finalizar Crédito Vigente", use_container_width=True
                    ):
                        try:
                            scope = [
                                "https://www.googleapis.com/auth/spreadsheets",
        ...
        # ------------------------------------------
            ...
            if not df_existente.empty:
                ...
                if meses_disponibles:
                    ...
                    # Gráfico de Barras de Rendimiento Mensual
